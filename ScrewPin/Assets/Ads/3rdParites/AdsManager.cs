...
public class AdsManager : MonoBehaviour
{
    ...
    [SerializeField] private string InterstitialAdUnitIdGeneric = "9d5b8afdc75942da";
    [SerializeField] private string RewardedAdUnitIdGeneric = "6efd8ca487b20ea3";
    [SerializeField] private string BannerAdUnitId = "d122e0da348a8dc9";
    [SerializeField] private string AppOpenAdUnitId = "";
    [SerializeField] private string RecID = "de1461194c05ffd1";
    ...
    public void ShowInter(Action onSuccess = null, Action onFail = null, string placement = "")
    {
        if (RemoveAds == 1)
        {
            Debug.Log("Ads removed. Skipping interstitial.");
            onSuccess?.Invoke();
            return;
        }
        ...
        if (MaxSdk.IsInterstitialReady(InterstitialAdUnitIdGeneric))
        {
            Debug.Log("Showing Interstitial with callback: " + placement);
            isAdShowing = true;

            _onInterstitialSuccess = onSuccess;
            _onInterstitialFail = onFail;

            // Attach temporary callbacks
            MaxSdkCallbacks.OnInterstitialHiddenEvent += HandleInterstitialClosed;
            MaxSdkCallbacks.OnInterstitialAdFailedToDisplayEvent += HandleInterstitialFailed;
    ...
    private Action _onInterstitialSuccess;
    private Action _onInterstitialFail;
    ...
    private void HandleInterstitialClosed(string adUnitId)
    {
        Debug.Log("Interstitial closed. Success callback called.");
        isAdShowing = false;

        MaxSdkCallbacks.OnInterstitialHiddenEvent -= HandleInterstitialClosed;
        MaxSdkCallbacks.OnInterstitialAdFailedToDisplayEvent -= HandleInterstitialFailed;

        _onInterstitialSuccess?.Invoke();
        _onInterstitialSuccess = null;
        _onInterstitialFail = null;

        StartCoroutine(PriorityInterstitialAdLoaded(InterstitialAdUnitIdGeneric));
    }

    private void HandleInterstitialFailed(string adUnitId, int errorCode)
    {
        Debug.LogWarning("Interstitial failed to show: " + errorCode);
        isAdShowing = false;

        MaxSdkCallbacks.OnInterstitialHiddenEvent -= HandleInterstitialClosed;
        MaxSdkCallbacks.OnInterstitialAdFailedToDisplayEvent -= HandleInterstitialFailed;

        _onInterstitialFail?.Invoke();
        _onInterstitialSuccess = null;
        _onInterstitialFail = null;
    ...
    Action RewardGiven;
    public void ShowReward(Action onSuccess, Action onFail = null, string placement = "")
    {
        if (MaxSdk.IsRewardedAdReady(RewardedAdUnitIdGeneric))
        {
            isAdShowing = true;
            RewardGiven = onSuccess;
